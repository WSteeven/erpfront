  ...
  <q-layout view="lHh Lpr lFf">
    ...
    <q-header class="bg-desenfoque">
      <q-toolbar class="row justify-between q-py-sm border-bottom">
        <q-btn v-if="route.name !== 'intranet'" dense aria-label="Menu" @click="toggleLeftDrawer"
          class="custom-shadow bg-primary" unelevated>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd"
              ...
              fill="#575b6e" id="path2" style="fill: #fff; stroke-width: 1.04881" />
            <path fill-rule="evenodd" clip-rule="evenodd"
              ...
              fill="#575b6e" id="path4" style="fill: #fff; stroke-width: 1.26625" />
            <path id="path6" style="fill: #fff; stroke-width: 1.04881"
        ...
        <img v-if="route.name === 'intranet'" :src="!$q.dark.isActive ? logoClaro : logoOscuro" height="30"
          class="custom-shadow" />
        <span class="row" :class="{
          ...
          <q-btn v-if="mostrarTransferirTareas" dense unelevated no-caps class="q-px-sm bg-grey-4d"
            ...
            <q-icon name="bi-arrow-left-right" :class="{ 'q-mx-sm': !$q.screen.xs }"
              class="bg-icon color-icon-navbar q-pa-xs rounded-field" size="xs"></q-icon>
            <span v-if="!$q.screen.xs" class="text-color">Transferir tareas activas</span>
            <q-tooltip class="bg-dark">Transferir tareas activas</q-tooltip>
          </q-btn>

          <!-- Boton movilizacion -->
          <q-btn dense unelevated no-caps class="q-px-sm" @click="abrirMovilizacionSubtarea()">
            <q-icon name="bi-car-front" :class="{ 'q-mr-sm': !$q.screen.xs }"
              class="bg-icon color-icon-navbar q-pa-xs rounded-field" size="xs"></q-icon>
          ...
          <!-- Boton Mi bodega -->
          <q-btn dense unelevated no-caps class="q-px-sm" :to="{ name: 'mi_bodega' }">
            <q-icon name="bi-box-seam" :class="{ 'q-mr-sm': !$q.screen.xs }"
              class="bg-icon color-icon-navbar q-pa-xs rounded-field" size="xs"></q-icon>
          ...
          <q-btn dense unelevated no-caps class="q-pl-sm" @click.self="mostrarNotificaciones = true">
            <q-icon name="bi-bell" :class="{ 'q-mr-sm': !$q.screen.xs }"
              class="bg-icon color-icon-navbar q-pa-xs rounded-field" size="xs"></q-icon>

            <!-- <span v-if="!$q.screen.xs">Notificaciones</span> -->
            <q-tooltip class="bg-dark">Notificaciones</q-tooltip>

            <q-badge v-if="notificaciones.length > 0" color="positive" floating><span>{{ notificaciones.length }}</span>
            </q-badge>

            <q-menu v-model="mostrarNotificaciones" :self="selfCenterMiddle" transition-show="slide-left"
              transition-hide="slide-right" :style="{ width: width }"
              class="window-height bg-desenfoque-2 custom-shadow" max-height="100vh" anchor="center middle">
              <div class="full-width text-right q-pr-md q-mb-md">
                <q-btn icon="bi-chevron-right" round dense unelevated color="primary" class="q-mt-sm"
                  @click="mostrarNotificaciones = false"></q-btn>
              </div>
              ...
              <q-list style="min-width: 140px; max-width: 450px" class="q-px-xs">
                ...
                <q-expansion-item v-for="titulo in Object.keys(notificacionesAgrupadas)" :key="titulo"
                  class="overflow-hidden q-mb-sm expansion"
                  :label="`${titulo} (${notificacionesAgrupadas[titulo].length})`"
                  header-class="text-bold bg-header-collapse text-primary full-width" icon="bi-dot">
                  <q-item v-for="notificacion in notificacionesAgrupadas[titulo]" :key="notificacion.id"
                    :to="notificacion.link">
                    <q-item-section avatar>
                      <q-icon color="grey-8" :name="obtenerIcono.obtener(notificacion.tipo_notificacion)
                        " />
                    </q-item-section>
                    ...
                    <q-item-section class="full-width">
                      ...
                      <q-item-label class="row justify-end q-pt-sm">
                        <q-btn icon="bi-check" label="Marcar como leído" dense color="positive" size="sm" outline
                          no-caps rounded unelevated @click="marcarLeida(notificacion.id)">
                        </q-btn></q-item-label>
                    </q-item-section>
                  </q-item>
                </q-expansion-item>
          ...
          <q-btn dense round flat glossy @click.self="mostrarMenu = true">
            ...
            <q-menu v-model="mostrarMenu" :self="selfCenterMiddle" transition-show="slide-left"
              transition-hide="slide-right" :style="{ width: width }" class="bg-desenfoque" max-height="100vh">
              <div class="column items-center q-py-sm window-height">
                <div class="full-width text-right q-pr-md">
                  <q-btn icon="bi-chevron-right" round dense unelevated color="primary"
                    @click="mostrarMenu = false"></q-btn>
                </div>

                <!-- <q-avatar size="72px" class="double-border q-mb-md"> -->
                <img :src="imagenPerfil" fit="contain" height="72px" width="72px"
                ...
                <q-item clickable :to="{ name: 'mi_bodega' }" class="full-width">
                  <q-avatar>
      ...
      <div class="text-center">
        <q-chip v-if="enCamino" class="bg-grey-2 q-mx-auto q-mb-md">
          <q-icon name="bi-car-front-fill" color="positive" class="q-mr-xs"></q-icon>
    ...
    <!-- Drawer -->
    <q-drawer v-model="leftDrawerOpen" class="bg-drawer border-right q-px-sm q-py-sm" show-if-above
      v-if="route.name !== 'intranet'">
      <!-- Drawer Header -->
      <div class="absolute-top q-pa-sm q-ma-sm rounded-card">
        <img :src="!$q.dark.isActive ? logoClaro : logoOscuro" height="60" class="q-mx-auto block" />
      </div>
      ...
      <q-scroll-area style="
        ...
        <q-list>
          <div v-for="item in links" :key="item.title">
            ...
            <EssentialLink v-else :title="item.title ?? ''" :link="item.link" :icon="item.icon"
              :children="item.children" :can="item.can"></EssentialLink>
          </div>
        </q-list>
      </q-scroll-area>

      <q-btn color="primary" no-caps class="full-width q-mb-md" outline rounded @click="logout()">Cerrar sesión</q-btn>
    </q-drawer>

...
import { ref } from 'vue'
import { AntecedenteTrabajoAnterior } from 'medico/gestionarPacientes/modules/fichaPeriodicaPreocupacional/domain/AntecedenteTrabajoAnterior'
...
export function useFichaPreocupacional(medicoStore, listadosAuxiliares) {
  ...
  const transformarAntecedentesEmpleosAnteriores = (
    antecedentes: AntecedenteTrabajoAnterior[]
  ): AntecedenteTrabajoAnterior[] => {
    const isNumericKey = (k: string) => /^\d+$/.test(k)
    const isTrueFlag = (v: unknown) =>
      v === true || v === 1 || v === '1' || v === 'true'
    return antecedentes.map(antecedente => {
      antecedente.tipos_riesgos_ids = Object.entries(antecedente)
        .filter(([k, v]) => isNumericKey(k) && isTrueFlag(v))
        .map(([k]) => Number(k))
      return antecedente
    })
  }

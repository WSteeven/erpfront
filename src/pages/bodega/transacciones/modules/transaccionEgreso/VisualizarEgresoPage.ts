...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const { notificarError, notificarCorrecto, confirmar, prompt } =
      useNotificaciones()
    //stores
    ...
    if (transaccionStore.transaccion) {
      ...
      listadoAux = JSON.parse(
        JSON.stringify(
          transaccionEgresoStore.transaccion.listadoProductosTransaccion
        )
      )
    }
    ...
    const esVisibleAutorizacion = ref(false)

    const esVisibleTarea = ref(false)
    const requiereFecha = ref(false) //para mostrar u ocultar fecha limite
    ...
    function aprobarEgreso() {
      const data: CustomActionPrompt = {
        ...
        accion: async data => {
          try {
            confirmar(
              'Esta acción firmará el comprobante de egreso ',
              async () => {
                ...
                const url =
                  apiConfig.URL_BASE +
                  '/' +
                  axios.getEndpoint(endpoints.comprobantes) +
                  '/' +
                  transaccion.id
    ...
    function aprobarEgresoParcial() {
      if (verificarRecibidoMenor())
        notificarError(
          'El valor de recibido no puede ser superior a la cantidad despachada'
        )
      else {
        const data: CustomActionPrompt = {
          ...
          accion: async data => {
            try {
              confirmar(
                'Esta acción firmará el comprobante de egreso con las cantidades y materiales aceptados ',
                async () => {
                  cargando.activar()
                  //aqui se aprueba y se firma el documento
                  const datos = {
                  ...
                  console.log(response)
                  notificarCorrecto(
                    'Documento parcial aprobado y firmado correctamente'
                  )
                  emit('cerrar-modal', false)
    ...
    function permitirModificarCantidades() {
      ...
      if (
        transaccion.modificar_recepcion &&
        transaccion.estado_comprobante === 'PARCIAL'
      ) {
        //primero se quita los completados
        transaccion.listadoProductosTransaccion =
          transaccion.listadoProductosTransaccion.filter(
            item => item.cantidad != item.recibido
          )
        ...
        transaccion.listadoProductosTransaccion = JSON.parse(
          JSON.stringify(listadoAux)
        )
      }
    }
    ...
    function verificarRecibidoMenor() {
      return transaccion.listadoProductosTransaccion.some(
        item => item.recibido > item.cantidad
      )
    }

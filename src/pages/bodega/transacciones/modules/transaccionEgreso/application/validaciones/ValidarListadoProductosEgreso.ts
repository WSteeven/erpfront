...
export class ValidarListadoProductosEgreso implements Validador {
  ...
  async validar(): Promise<boolean> {
    ...
    const tiene_ceros = this.transaccion.listadoProductosTransaccion.some(
      v => v.cantidad <= 0
    )
    if (tiene_ceros)
      throw new Error(
        'La cantidad de los productos del listado debe ser mayor a 0'
      )
    ...
    this.listadoPedido.value.forEach(v => {
      ...
      if (v.cantidad > 0)
        if (total > v.cantidad)
          throw new Error(
            'No puedes despachar más items de la cantidad solicitada'
          )

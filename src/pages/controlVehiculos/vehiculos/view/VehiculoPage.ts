...
export default defineComponent({
  ...
  setup() {
    ...
    const { onReestablecer, onGuardado, onConsultado, onModificado } =
      mixin.useHooks()
    ...
    const reglas = {
      ...
      num_motor: { required },
      num_chasis: { required },
      anio_fabricacion: { required, maxLength: maxLength(4) },
      ...
      conductor_externo: {
        requiredIf: requiredIf(() => vehiculo.tipo == 'ALQUILADO')
      },
    ...
    async function recargarSeguros() {
      cargando.activar()
      listadosAuxiliares.seguros = (
        await new SeguroVehicularController().listar({
          estado: 1,
          'fecha_caducidad[operator]': '>',
          'fecha_caducidad[value]': obtenerFechaActual(maskFecha)
        })
    ...
    function seleccionarMarca(val) {
      ...
      if (encontrado.length > 0) {
        marcas.value = listadosAuxiliares.marcas.filter(

...
export default defineComponent({
  ...
  setup() {
    ...
    function checkValueFechaMatricula(val, reason) {
      is_month_fecha_matricula.value = reason !== 'month'
    }

    function checkValueProximaMatricula(val, reason) {
      is_month_proxima_matricula.value = reason !== 'month'
    }
    ...
    function asignarPlaca() {
      const vehiculoSeleccionado = vehiculos.value.filter(
        (v: Vehiculo) => v.id == matricula.vehiculo
      )
      ...
      accion: ({ entidad }) => {
        const data: CustomActionPrompt = {
          ...
          accion: async data => {
            ...
            const result = await matriculaStore.setValorEstimadoPagar({
              valor_estimado_pagar: data
            })
            console.log(result)
    ...
    const btnPagarMatricula: CustomActionTable = {
      ...
      accion: ({ entidad }) => {
        confirmar(
          '¿Está seguro de marcar como pagada la matrícula?',
          async () => {
            const data: CustomActionPrompt = {
                  ...
                  accion: async data2 => {
                    ...
                    const data3: CustomActionPrompt = {
                      ...
                      accion: async data => {
                        ...
                        if (
                          await matriculaStore.pagarMatricula(dataPagoMatricula)
                        ) {
                          await filtrarMatriculas('3')
                        }
                      }
                    }
                    prompt(data3)
    ...
    return {
      ...
      maskFecha: 'MM-YYYY',
      is_month_fecha_matricula,
      is_month_proxima_matricula,

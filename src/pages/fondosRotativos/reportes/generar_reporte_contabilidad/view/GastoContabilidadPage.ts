...
import { GastoContabilidadController } from '../infrestructure/GastoContabilidadController'
import { configuracionColumnasGastoContabilidad } from '../domain/configuracionColumnasGastoContabilidad'
...
export default defineComponent({
  components: { EssentialTable, SelectorImagen },
  setup() {
    /*********
     * Stores
     *********/
     ...
     * Mixin
     ************/
    const mixin = new ContenedorSimpleMixin(GastoContabilidad, new GastoContabilidadController())
    const {
      entidad: gasto_contabilidad,
      disabled,
      accion,
      listadosAuxiliares,
      listado,
    ...
    const v$ = useVuelidate(reglas, gasto_contabilidad)
      ...
      if (val === '') {
        update(() => {
          sub_detalles.value = listadosAuxiliares.sub_detalles.filter(
            (v) => v.id_detalle_viatico == gasto_contabilidad.detalle
          )
        })
        return
      }
    ...
    function filtrarTareas(val, update) {
      if (gasto_contabilidad.proyecto == 0) {
        update(() => {
      ...
      if (val === '') {
        update(() => {
          tareas.value = listadosAuxiliares.tareas.filter(
            (v) => v.proyecto_id == gasto_contabilidad.proyecto
          )
        })
        return
      }
    ...
    watchEffect(() => (gasto_contabilidad.total = gasto_contabilidad.cantidad! * gasto_contabilidad.valor_u!))
    ...
    return {
      ...
      accion,
      v$,
      configuracionColumnasGastoContabilidad,
      autorizacionesEspeciales,
      watchEffect,

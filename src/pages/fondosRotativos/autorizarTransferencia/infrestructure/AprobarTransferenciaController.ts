...
export class AprobarTransferenciaController  {
 ...
 async aprobarTransferencia(transferencia: Transferencia): Promise<void> {
  try {
    await this.axios.post(this.axios.getEndpoint(endpoints.aprobar_trnasferencia), transferencia)
  } catch (error: unknown) {
  ...
  async rechazarTransferencia(transferencia: Transferencia): Promise<void> {
    try {
      await this.axios.post(this.axios.getEndpoint(endpoints.rechazar_transferencia), transferencia)
    } catch (error: unknown) {
      const axiosError = error as AxiosError
      throw new ApiError(axiosError)
    }

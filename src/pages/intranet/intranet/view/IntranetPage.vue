  ...
  <q-header class="bg-desenfoque">
    ...
    <div class="row items-center justify-start">
      <div class="absolute-top q-pa-sm q-ma-sm rounded-card">
        <img :src="!$q.dark.isActive ? logoClaro : logoOscuro" height="30" class="q-mx-auto" />
      </div>
      ...
      <q-toolbar class="row items-center justify-end q-py-sm border-bottom">
        <span class="row" :class="{
          ...
          <q-btn dense round flat glossy @click.self="mostrarMenu = true">
            ...
            <q-menu v-model="mostrarMenu" :self="selfCenterMiddle" transition-show="jump-down"
              transition-hide="jump-out" :style="{ 'min-width': width }"
              class="window-height bg-desenfoque custom-shadow" max-height="100vh">
              <div class="column items-center q-py-sm window-height">
                <div class="full-width text-right q-pr-md">
                  <q-btn icon="bi-x" round dense unelevated outline color="grey-8" @click="mostrarMenu = false"></q-btn>
                </div>
  ...
  <q-page padding>
    ...
    <div class="row q-col-gutter-sm q-ml-sm q-pl-sm">
      <div class="col-6 col-md-3">
        <label class="q-mb-sm block"><strong>Noticias Programadas</strong></label>
        <q-date v-model="date" :events="eventos" @update:model-value="verEvento(date)"
          :event-color="(date) => (date[9] % 2 === 0 ? 'teal' : 'orange')" minimal style="width: max-content" />
        <q-card class="solicitudes q-my-md">
          ...
          <q-card-section>
            ...
            <q-input v-model="solicitud.descripcion" label="Descripción" outlined type="textarea" rows="3" />
          </q-card-section>
      ...
      <div class="col-3 col-md-6">
        ...
        <div class="row">
          <div class="col-6 q-pa-md" v-for="(modulo, index) in modulosPermitidos" :key="index">
            <q-card style="width: 300px; height: 200px;">
              <div class="text-center">
                <q-btn outline color="primary" :label="modulo" @click="goToModule(modulo)"
                  size="lg" />
              </div>
            </q-card>
          </div>

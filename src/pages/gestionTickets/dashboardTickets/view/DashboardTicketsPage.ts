  ...
  setup() {
    ...
    cargarVista(async () => {
      await obtenerListados({
        empleados: {
          ...
          params: {
            campos:
              'id,nombres,apellidos,departamento_id,responsable_departamento',
            estado: 1
          }
        },
    ...
    const mostrarTitulosSeccion = computed(
      () =>
        filtro.fecha_inicio &&
        filtro.fecha_fin &&
        (filtro.empleado || filtro.departamento)
    )
    ...
    const mostrarSeccionDepartamento = computed(
      () =>
    ...
    const reglas = {
      ...
      departamento: {
        requiredIf: requiredIf(() => mostrarSeccionDepartamento.value)
      }
    }
      ...
      if (await v$.value.$validate()) {
        try {
          ...
          const { result } = await dashboardTicketController.listar({
            ...
            empleado_id: filtro.empleado
          }) //, departamento_responsable_id: departamento })
          ...
          cantTicketsCreadosADepartamentos.value =
            result.cantTicketsCreadosADepartamentos
          ...
          cantTicketsFinalizadosSolucionados.value =
            result.cantTicketsFinalizadosSolucionados
          cantTicketsFinalizadosSinSolucion.value =
            result.cantTicketsFinalizadosSinSolucion
          cantTicketsCalificadosResponsable.value =
            result.cantTicketsCalificadosResponsable
          cantTicketsCalificadosSolicitante.value =
            result.cantTicketsCalificadosSolicitante

          // Grafico empleado consultado
          ticketsPorEstado.value = result.ticketsPorEstado
          const graficoTicketsPorEstado = contarTicketsEmpleado(
            result.ticketsPorEstado
          )
          // console.log(graficoTicketsPorEstado)
          const labels3 = graficoTicketsPorEstado.map(item => item.estado)
          const valores3 = graficoTicketsPorEstado.map(
            item => item.total_tickets
          )
          const colores3 = graficoTicketsPorEstado.map(item =>
            mapearColor(item.estado)
          )
          ticketsPorEstadoBar.value = mapearDatos(
            labels3,
            valores3,
            'Cantidad de tickets',
            colores3
          ...
          cantidadesTicketsSolicitadosPorDepartamento.value =
            result.ticketsCreadosADepartamentos
          const graficoTicketsCreadosDepartamento = contarTicketsDepartamento(
            result.ticketsCreadosADepartamentos
          )
          const labels = graficoTicketsCreadosDepartamento.map(
            item => item.departamento_responsable
          )
          const valores = graficoTicketsCreadosDepartamento.map(
            item => item.total_tickets
          )
          const colores1 = graficoTicketsCreadosDepartamento.map(item =>
            generarColorAzulPastelClaro()
          ) //mapearColorDepartamentos(item.departamento_responsable))
          cantidadesTicketsSolicitadosPorDepartamentoBar.value = mapearDatos(
            labels,
            valores,
            'Cantidad de tickets creados a los departamentos',
            colores1
          )

          cantidadesTicketsRecibidosPorDepartamento.value =
            result.ticketsRecibidosPorDepartamentos
          const graficoTicketsRecibidosDepartamento =
            contarTicketsDepartamentoSolicitante(
              result.ticketsRecibidosPorDepartamentos
            )
          const labels2 = graficoTicketsRecibidosDepartamento.map(
            item => item.departamento_solicitante
          )
          const valores2 = graficoTicketsRecibidosDepartamento.map(
            item => item.total_tickets
          )
          const colores2 = graficoTicketsRecibidosDepartamento.map(item =>
            generarColorAzulPastelClaro()
          ) //mapearColorDepartamentos(item.departamento_solicitante))
          cantidadesTicketsRecibidosPorDepartamentoBar.value = mapearDatos(
            labels2,
            valores2,
      ...
      if (await v$.value.$validate()) {
        ...
        try {
          const { result } = await dashboardTicketController.listar({
            fecha_inicio: filtro.fecha_inicio,
            fecha_fin: filtro.fecha_fin,
            departamento_responsable_id: filtro.departamento
          })
          // console.log(result)

          // Grafico de pastel
          // Graficos estadisticos del empleado
          ticketsPorDepartamentoEstadoAsignado.value =
            await result.ticketsPorDepartamentoEstadoAsignado
          const graficoTicketsPorDepartamentoEstadoAsignado =
            contarTicketsResponsable(
              result.ticketsPorDepartamentoEstadoAsignado
            ) // <--
          const labels4 = graficoTicketsPorDepartamentoEstadoAsignado.map(
            item => item.responsable
          )
          const valores4 = graficoTicketsPorDepartamentoEstadoAsignado.map(
            item => item.total_tickets
          )
          const colores4 = graficoTicketsPorDepartamentoEstadoAsignado.map(() =>
            generarColorAzulPastelClaro()
          )
          ticketsPorDepartamentoEstadoAsignadoBar.value = mapearDatos(
            labels4,
            valores4,
            'Cantidad de tickets del departamento con filtro por estado',
            colores4
          )

          ticketsPorDepartamentoEstadoRechazado.value =
            await result.ticketsPorDepartamentoEstadoRechazado
          const graficoTicketsPorDepartamentoEstadoRechazado =
            contarTicketsResponsable(
              result.ticketsPorDepartamentoEstadoRechazado
            ) // <--
          const labels12 = graficoTicketsPorDepartamentoEstadoRechazado.map(
            item => item.responsable
          )
          const valores12 = graficoTicketsPorDepartamentoEstadoRechazado.map(
            item => item.total_tickets
          )
          const colores12 = graficoTicketsPorDepartamentoEstadoRechazado.map(
            () => generarColorAzulPastelClaro()
          )
          ticketsPorDepartamentoEstadoRechazadoBar.value = mapearDatos(
            labels12,
            valores12,
            'Cantidad de tickets del departamento con filtro por estado',
            colores12
          ...
          ticketsPorDepartamentoEstadoReasignado.value =
            result.ticketsPorDepartamentoEstadoReasignado
          const graficoTicketsPorDepartamentoEstadoReasignado =
            contarTicketsResponsable(
              result.ticketsPorDepartamentoEstadoReasignado
            )
          const labels5 = graficoTicketsPorDepartamentoEstadoReasignado.map(
            item => item.responsable
          )
          const valores5 = graficoTicketsPorDepartamentoEstadoReasignado.map(
            item => item.total_tickets
          )
          const colores5 = graficoTicketsPorDepartamentoEstadoReasignado.map(
            () => generarColorAzulPastelClaro()
          )
          ticketsPorDepartamentoEstadoReasignadoBar.value = mapearDatos(
            labels5,
            valores5,
            'Cantidad de tickets del departamento con filtro por estado',
            colores5
          ...
          ticketsPorDepartamentoEstadoEjecutando.value =
            result.ticketsPorDepartamentoEstadoEjecutando
          const graficoTicketsPorDepartamentoEstadoEjecutando =
            contarTicketsResponsable(
              result.ticketsPorDepartamentoEstadoEjecutando
            )
          const labels6 = graficoTicketsPorDepartamentoEstadoEjecutando.map(
            item => item.responsable
          )
          const valores6 = graficoTicketsPorDepartamentoEstadoEjecutando.map(
            item => item.total_tickets
          )
          const colores6 = graficoTicketsPorDepartamentoEstadoEjecutando.map(
            () => generarColorAzulPastelClaro()
          )
          ticketsPorDepartamentoEstadoEjecutandoBar.value = mapearDatos(
            labels6,
            valores6,
            'Cantidad de tickets del departamento con filtro por estado',
            colores6
          ...
          ticketsPorDepartamentoEstadoPausado.value =
            result.ticketsPorDepartamentoEstadoPausado
          const graficoTicketsPorDepartamentoEstadoPausado =
            contarTicketsResponsable(result.ticketsPorDepartamentoEstadoPausado)
          const labels7 = graficoTicketsPorDepartamentoEstadoPausado.map(
            item => item.responsable
          )
          const valores7 = graficoTicketsPorDepartamentoEstadoPausado.map(
            item => item.total_tickets
          )
          const colores7 = graficoTicketsPorDepartamentoEstadoPausado.map(() =>
            generarColorAzulPastelClaro()
          )
          ticketsPorDepartamentoEstadoPausadoBar.value = mapearDatos(
            labels7,
            valores7,
            'Cantidad de tickets del departamento con filtro por estado',
            colores7
          ...
          listados.ticketsPorDepartamentoEstadoFinalizadoSolucionado =
            result.ticketsPorDepartamentoEstadoFinalizadoSolucionado
          ticketsPorDepartamentoEstadoFinalizadoSolucionado.value =
            result.ticketsPorDepartamentoEstadoFinalizadoSolucionado
          const graficoTicketsPorDepartamentoEstadoFinalizadoSolucionado =
            contarTicketsResponsable(
              result.ticketsPorDepartamentoEstadoFinalizadoSolucionado
            )
          const labels8 =
            graficoTicketsPorDepartamentoEstadoFinalizadoSolucionado.map(
              item => item.responsable
            )
          const valores8 =
            graficoTicketsPorDepartamentoEstadoFinalizadoSolucionado.map(
          ...
          ticketsPorDepartamentoEstadoFinalizadoSolucionadoBar.value =
            mapearDatos(
              labels8,
              valores8,
              'Cantidad de tickets del departamento con filtro por estado',
          ...
          ticketsPorDepartamentoEstadoFinalizadoSinSolucion.value =
            result.ticketsPorDepartamentoEstadoFinalizadoSinSolucion
          const graficoTicketsPorDepartamentoEstadoFinalizadoSinSolucion =
            contarTicketsResponsable(
              result.ticketsPorDepartamentoEstadoFinalizadoSinSolucion
            )
          const labels9 =
            graficoTicketsPorDepartamentoEstadoFinalizadoSinSolucion.map(
              item => item.responsable
            )
          const valores9 =
            graficoTicketsPorDepartamentoEstadoFinalizadoSinSolucion.map(
          ...
          ticketsPorDepartamentoEstadoFinalizadoSinSolucionBar.value =
            mapearDatos(
              labels9,
              valores9,
              'Cantidad de tickets del departamento con filtro por estado',
          ...
          ticketsPorDepartamentoEstadoCalificado.value =
            result.ticketsPorDepartamentoEstadoCalificado
          const labels10 = result.ticketsPorDepartamentoEstadoCalificado.map(
            item => item.responsable
          )
          const valores10 = result.ticketsPorDepartamentoEstadoCalificado.map(
            item => item.total_tickets
          )
          const colores10 = result.ticketsPorDepartamentoEstadoCalificado.map(
            () => generarColorAzulPastelClaro()
          )
          ticketsPorDepartamentoEstadoCalificadoBar.value = mapearDatos(
            labels10,
            valores10,
            'Cantidad de tickets del departamento con filtro por estado',
            colores10
          ...
          listados.ticketsPorDepartamentoEstadoFinalizadoSolucionado =
            result.ticketsPorDepartamentoEstadoFinalizadoSolucionado
          const labels11 =
            result.ticketsPorDepartamentoEstadoFinalizadoSolucionado.map(
              item => item.codigo
            )
          const valores11 =
            result.ticketsPorDepartamentoEstadoFinalizadoSolucionado.map(
              item => item.tiempo_hasta_finalizar_horas
            )
          const colores11 =
            result.ticketsPorDepartamentoEstadoFinalizadoSolucionado.map(item =>
              generarColorAzulPastelClaro()
            )
          promedioTiemposLine.value = mapearDatos(
            labels11,
            valores11,
            'Tiempo ocupado (h)',
            colores11
    ...
    function mapearColor(estadoTicket: keyof typeof estadosTickets) {
      switch (estadoTicket) {
        ...
        case estadosTickets.FINALIZADO_SIN_SOLUCION:
          return '#9ba98c'
        case estadosTickets.FINALIZADO:
          return '#8bc34a'
        case estadosTickets.CANCELADO:
          return '#c31d25'
      }
    }
      ...
      if (label) {
        switch (categoriaGrafico) {
          case categoriaGraficosEmpleado.ESTADO_ACTUAL:
            ticketsPorEstadoListado.value = ticketsPorEstado.value.filter(
              (ticket: Ticket) => ticket.estado === label
            )
            // tabsEmpleado.value = opcionesEmpleado.empleadoListado
            break
          case categoriaGraficosEmpleado.CREADOS_A_DEPARTAMENTOS:
            ticketsPorEstadoListado.value =
              cantidadesTicketsSolicitadosPorDepartamento.value.filter(
                (ticket: Ticket) => ticket.departamento_responsable === label
              )
            // tabsEmpleado.value = opcionesEmpleado.empleadoListado
            break
          case categoriaGraficosEmpleado.ASIGNADOS_POR_DEPARTAMENTOS:
            ticketsPorEstadoListado.value =
              cantidadesTicketsRecibidosPorDepartamento.value.filter(
                (ticket: Ticket) => ticket.departamento_solicitante === label
              )
            break
        }
        tabsEmpleado.value = opcionesEmpleado.empleadoListado
    ...
    function clickGraficoTicketsDepartamento(
      data,
      estado: keyof typeof estadosTickets
    ) {
      const { label } = data
      if (label) {
        switch (estado) {
          case estadosTickets.ASIGNADO:
            ticketsEmpleadoResponsable.value =
              ticketsPorDepartamentoEstadoAsignado.value.filter(
                (ticket: Ticket) => ticket.responsable === label
              )
            break
          case estadosTickets.RECHAZADO:
            ticketsEmpleadoResponsable.value =
              ticketsPorDepartamentoEstadoRechazado.value.filter(
                (ticket: Ticket) => ticket.responsable === label
              )
            break
          case estadosTickets.EJECUTANDO:
            ticketsEmpleadoResponsable.value =
              ticketsPorDepartamentoEstadoEjecutando.value.filter(
                (ticket: Ticket) => ticket.responsable === label
              )
            break
          case estadosTickets.PAUSADO:
            ticketsEmpleadoResponsable.value =
              ticketsPorDepartamentoEstadoPausado.value.filter(
                (ticket: Ticket) => ticket.responsable === label
              )
            break
          case estadosTickets.REASIGNADO:
            ticketsEmpleadoResponsable.value =
              ticketsPorDepartamentoEstadoReasignado.value.filter(
                (ticket: Ticket) => ticket.responsable === label
              )
            break
          case estadosTickets.FINALIZADO_SOLUCIONADO:
            ticketsEmpleadoResponsable.value =
              ticketsPorDepartamentoEstadoFinalizadoSolucionado.value.filter(
                (ticket: Ticket) => ticket.responsable === label
              )
            break
          case estadosTickets.FINALIZADO_SIN_SOLUCION:
            ticketsEmpleadoResponsable.value =
              ticketsPorDepartamentoEstadoFinalizadoSinSolucion.value.filter(
                (ticket: Ticket) => ticket.responsable === label
              )
            break
        }
    ...
    function clickGraficoLineaTiempo(data) {
      ...
      const config: CustomActionPrompt = reactive({
        ...
        accion: async opcion => {
          ticketStore.filaTicket =
            listados.ticketsPorDepartamentoEstadoFinalizadoSolucionado.filter(
              (ticket: Ticket) => ticket.codigo === data.label
            )[0]
          switch (opcion) {
            case 'MAS_DETALLES':
              modales.abrirModalEntidad('DetalleCompletoTicket')
    ...
    function contarTicketsDepartamento(tickets: Ticket[]): any[] {
      const conteo = tickets.reduce((acumulador: any, ticket) => {
        ...
        if (departamento_responsable) {
          const elementoExistente: any = acumulador.find(
            (item: any) =>
              item.departamento_responsable === departamento_responsable
          )

          if (!elementoExistente)
            acumulador.push({ departamento_responsable, total_tickets: 1 })
          else elementoExistente.total_tickets++
        }
    ...
    function contarTicketsDepartamentoSolicitante(tickets: Ticket[]): any[] {
      const conteo = tickets.reduce((acumulador: any, ticket) => {
        ...
        const elementoExistente: any = acumulador.find(
          (item: any) =>
            item.departamento_solicitante === departamento_solicitante
        )

        if (!elementoExistente)
          acumulador.push({ departamento_solicitante, total_tickets: 1 })
        else elementoExistente.total_tickets++
    ...
    function contarTicketsResponsable(tickets: Ticket[]): any[] {
      const conteo = tickets.reduce((acumulador: any, ticket) => {
        ...
        const elementoExistente: any = acumulador.find(
          (item: any) => item.responsable === responsable
        )

        if (!elementoExistente)
          acumulador.push({ responsable, total_tickets: 1 })
        else elementoExistente.total_tickets++

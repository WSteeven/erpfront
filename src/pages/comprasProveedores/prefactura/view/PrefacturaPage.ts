...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const descuento = computed(() =>
      prefactura.descuento_general == 0
        ? prefactura.listadoProductos
            .reduce((prev, curr) => prev + parseFloat(curr.descuento), 0)
            .toFixed(2)
        : prefactura.descuento_general
    )
    const iva = computed(() =>
      ((subtotal_con_impuestos.value * prefactura.iva) / 100).toFixed(2)
    )
    ...
    //Filtros y listados
    const { clientes, filtrarClientes, ordenarClientes } =
    ...
    function eliminar({ posicion }) {
      confirmar('¿Está seguro de continuar?', () =>
        prefactura.listadoProductos.splice(posicion, 1)
      )
    }

    function actualizarProforma() {
      if (!prefactura.proforma || prefactura.proforma === 0) limpiarPrefactura()
    }
    /**
    ...
    async function cargarDatosProforma() {
      ...
      prefactura.solicitante = Number.isInteger(
        proformaStore.proforma.solicitante
      )
        ? proformaStore.proforma.solicitante
        : proformaStore.proforma.solicitante_id
      prefactura.cliente = Number.isInteger(proformaStore.proforma.cliente)
        ? proformaStore.proforma.cliente
        : proformaStore.proforma.cliente_id
        ...
        confirmar('¿Está seguro de anular la prefactura?', () => {
          const data: CustomActionPrompt = {
            ...
            accion: async data => {
              try {
                prefacturaStore.idPrefactura = entidad.id
                const response = await prefacturaStore.anularPrefactura({
                  motivo: data
                })

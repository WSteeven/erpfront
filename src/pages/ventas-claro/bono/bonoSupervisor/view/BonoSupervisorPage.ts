...
import { BonoSupervisorController } from '../infrestucture/BonoSupervisorController'
import { configuracionColumnasBonoSupervisor } from '../domain/configuracionColumnasBonoSupervisor'
...
export default defineComponent({
  ...
  setup() {
    const mixin = new ContenedorSimpleMixin(
      BonoSupervisor,
      new BonoSupervisorController()
    )
    const { entidad: bono_supervisor, accion, disabled } = mixin.useReferencias()
    ...
    const v$ = useVuelidate(reglas, bono_supervisor)
    ...
    return {
      ...
      disabled,
      maskFecha,
      configuracionColumnas: configuracionColumnasBonoSupervisor,
    }
  },
})

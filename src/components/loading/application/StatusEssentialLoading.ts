...
export class StatusEssentialLoading {
  ...
  waitBackground = this.$q.dark.isActive ? 'bg-grey-10' : 'bg-grey-4'
  activar(): void {
    // this.store.activarCargando()
    this.$q.loading.show({
      spinner: undefined,//QSpinnerBars,
      // spinnerColor: 'primary',
      message: this.spinner + '<span class="text-primary text-italic">La operación está en progreso.<br/><br/><small class=" text-italic q-py-xs q-px-sm ' + this.waitBackground + ' rounded-card">Espere por favor...</small></span>',
      html: true,

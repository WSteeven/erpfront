...
export const useTransaccionStore = defineStore('transaccion', () => {
  ...
  async function imprimirIngreso() {
    ...
    const url =
      apiConfig.URL_BASE +
      '/' +
      axios.getEndpoint(endpoints.transacciones_ingresos) +
      '/imprimir/' +
      idTransaccion.value
  ...
  async function imprimirActaEntregaRecepcion() {
    ...
    const url =
      ...
      '/' +
      axios.getEndpoint(endpoints.transacciones_egresos) +
      '/imprimir-entrega-recepcion/' +
      idTransaccion.value
    const filename =
      'acta_entrega_recepcion_' + idTransaccion.value + '_' + Date.now()
    await imprimirArchivo(url, 'GET', 'blob', 'pdf', filename)
  }
  ...
  async function editarItemEgreso(data) {
    try {
      ...
      const ruta =
        apiConfig.URL_BASE +
        '/' +
        axios.getEndpoint(endpoints.modificar_item_egreso) +
        '/' +
        idTransaccion.value
      const response: AxiosResponse = await axios.patch(ruta, data)
      console.log(response)
      if (response.status === 200)
        notificaciones.notificarCorrecto('Item actualizado correctamente')
    } catch (error: any) {
  ...
  async function quitarItemIngreso(data: any) {
    try {
      statusLoading.activar()
      const axios = AxiosHttpRepository.getInstance()
      const ruta =
        apiConfig.URL_BASE +
        '/' +
        axios.getEndpoint(endpoints.quitar_item_ingreso) +
        '/' +
        idTransaccion.value
      const response: AxiosResponse = await axios.patch(ruta, data)
      console.log(response)
      if (response.status === 200) {
        notificaciones.notificarCorrecto('Item eliminado correctamente')
        return true
      }
    } catch (error: any) {
